...
``dashboard.serve`` (pipeline/web), and `finetune` delegates to ``run_finetune.main``
(agents/tools/common/run_finetune.py — the standalone low-level tool).

    python agents/vla-trainbot/trainbot.py run                 # resume (or start) the full pipeline
    python agents/vla-trainbot/trainbot.py run --reset         # wipe state, run from the beginning
    python agents/vla-trainbot/trainbot.py run --profile n1d5  # pick a training profile (default: config)
    python agents/vla-trainbot/trainbot.py status              # print pipeline state and exit
    python agents/vla-trainbot/trainbot.py stop                # terminate the detached training run
    python agents/vla-trainbot/trainbot.py dashboard           # read-only live web dashboard
    python agents/vla-trainbot/trainbot.py finetune [run|monitor|watch|download|status|stop|selftest]
...
def _find_workspace_root() -> Path:
    ...
    for d in (HERE, *HERE.parents):
        if (d / "workspace.yaml").is_file():
            return d
    return HERE.parent                                   # legacy fallback (scripts/ -> root)


ROOT = _find_workspace_root()                            # workspace root
sys.path.insert(0, str(ROOT))                            # project-level modules (if any)
sys.path.insert(0, str(ROOT / "agents" / "tools"))       # workspace_paths helper
from workspace_paths import ws_path                      # noqa: E402

sys.path.insert(0, str(ws_path("common_tools")))         # pegasus / run_finetune / wifi_switch
for _sub in ("core", "stages", "web"):                   # pipeline_* + stage modules + dashboard
    sys.path.insert(0, str(ws_path("vla_harness") / _sub))


def main() -> None:
    ap = argparse.ArgumentParser(prog="trainbot.py", description=__doc__,
