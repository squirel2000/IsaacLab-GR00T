...
> Workspace layout: **[../../../README.md](../../../README.md)** · agent runbook: **[../README.md](../README.md)**.

## One entry point

```powershell
copy agents\vla-trainbot\harness\config\config.example.yaml agents\vla-trainbot\harness\config\config.yaml   # edit paths/passwords
python agents/vla-trainbot/trainbot.py run                 # resume (or start) the full pipeline  ← default
python agents/vla-trainbot/trainbot.py run --reset         # wipe state, run from IDLE
python agents/vla-trainbot/trainbot.py run --profile n1d5  # pick a training profile
python agents/vla-trainbot/trainbot.py status              # print state and exit (no network)
python agents/vla-trainbot/trainbot.py stop                # terminate the detached training run
python agents/vla-trainbot/trainbot.py dashboard           # read-only live web UI at http://localhost:8770
python agents/vla-trainbot/trainbot.py finetune […]        # fine-tune-only tool (agents/tools/common/run_finetune.py)
```

`agents/vla-trainbot/trainbot.py` is a thin dispatcher — one shared implementation per behavior:
...
| EVAL | `stages/eval_runner` | detached `agents/evalbot/harness/run_eval.py` on the H100 → success rate |
...
2. Edit **`scripts/pipeline/config/config.yaml`** → under `eval:` set:
   ...
   ```
3. Run as usual: `python agents/vla-trainbot/trainbot.py run` — EVAL now runs after TRAINING.

To eval an existing checkpoint **standalone** instead (no pipeline): edit
`scripts/eval/configs/eval_config.yaml` and run `python agents/evalbot/harness/run_eval.py`.
...
python agents/vla-trainbot/trainbot.py status
```
